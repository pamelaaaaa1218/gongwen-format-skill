...
LINE_MULT     = 1.25  # 全文行距：1.25 倍行距
LINE_BASE_PT  = 24    # 段前/段后「N 行」折算磅值的基准（三号字 1.25 倍行距约 24 磅）
                      # 说明：段前段后以 beforeLines/afterLines「N 行」为主，Word/WPS 按实际
                      # 行高计算；这里的磅值只是给不认这两个属性的渲染器兜底。
...
def _has_ambiguous_cjk(text):
    """run 里是否含非 ASCII 字符（汉字、中文引号“”、省略号…、破折号—、全角标点等）。
    这类字符 Word 要靠 w:hint 决定走东亚字体还是西文字体；缺 hint 会被当西文，
    中文引号就会渲染成 Times New Roman 的英文引号（又方又粗）。"""
    return any(ord(ch) > 0x7F for ch in text)


def _set_run_fonts(rfonts, font_name, text):
    """写 rFonts：中文（东亚）用 font_name，数字/英文（西文）用 FONT_WESTERN。
    含中文/中文标点的 run additionally 加 hint=eastAsia，让两可字符跟随中文字体；
    纯 ASCII 的 run 不加 hint，数字英文照常走西文字体。"""
    rfonts.set(qn('w:eastAsia'), font_name)
    rfonts.set(qn('w:ascii'), FONT_WESTERN)
    rfonts.set(qn('w:hAnsi'), FONT_WESTERN)
    rfonts.set(qn('w:cs'), FONT_WESTERN)
    if _has_ambiguous_cjk(text):
        rfonts.set(qn('w:hint'), 'eastAsia')
    elif rfonts.get(qn('w:hint')) is not None:
...
def set_run(run, font_name, size_pt):
    ...
    if rfonts is None:
        ...
        rpr.insert(0, rfonts)
    _set_run_fonts(rfonts, font_name, run.text)
...
def set_line_spacing(p, pt=None, mult=None, before_lines=0, after_lines=0):
    """设置行距：传 pt 用固定值（磅，lineRule=exact）；传 mult 用倍数行距（lineRule=auto）。
    段前 before_lines 行、段后 after_lines 行（0 = 无）——「N 行」用 beforeLines/afterLines
    （Word/WPS 段落设置里显示为「N 行」），再折算 twip 兜底。"""
    ...
    if spacing is None:
        ...
        pPr.append(spacing)
    if mult:
        # 倍数行距：240 twip = 单倍，Word 里显示为「多倍行距 N」
        spacing.set(qn('w:line'), str(int(240 * mult)))
    ...
    base_pt = pt if pt else LINE_BASE_PT
    _set_before(spacing, base_pt, before_lines)
    _set_after(spacing, base_pt, after_lines)
...
def apply_table_spacing(doc, before_lines=TABLE_BEFORE_LINES):
    ...
    for tbl in body.findall(qn('w:tbl')):
        ...
        if prev is not None and prev.tag == qn('w:p'):
            set_para_after(prev, LINE_BASE_PT, before_lines)
            n += 1
    return n
...
def format_document(doc, roles, texts):
    ...
    for i, p in enumerate(doc.paragraphs):
        ...
        set_line_spacing(p, mult=LINE_MULT, before_lines=before, after_lines=after)
        if role == 'empty':
            continue
    ...
    # 表格：仅统一字体，不改字号/结构
    for table in doc.tables:
        table.alignment = WD_TABLE_ALIGNMENT.CENTER   # 表格整体在页面居中
        for row in table.rows:
            for cell in row.cells:
                for p in cell.paragraphs:
                    for run in p.runs:
...
def main():
    ...
    print()
    print(f"✅ 已生成：{out}")
    print("   字体/字号、首行缩进、页边距(A4·上3.7/下3.5/左2.8/右2.6cm)均已套用。")
    print("   行距：全文 1.25 倍行距。")
    print("   数字与英文已统一为 Times New Roman，中文保持仿宋/黑体/楷体；")
    print("   中文引号等两可字符已加东亚提示，随中文字体显示（不再被西文字体渲染）。")
    print("   间距：大标题段后1.5行、一级标题段前1行、二级标题段前0.5行。")
    if table_gaps:
        print(f"   表格：{table_gaps} 处表格已设段前0.5行（落在表格上方段落的段后）、表格整体居中。")
    if num_plan:
